...
salesRank AS (
  SELECT 
    market_date,
    totalSales,
    RANK() OVER (ORDER BY totalSales DESC) AS bestRank,
    RANK() OVER (ORDER BY totalSales ASC) AS worstRank
  FROM salesDate
)
SELECT market_date, totalSales, 'Best Day' AS label FROM salesRank
WHERE bestRank = 1

UNION

SELECT market_date, totalSales, 'Worst Day' AS label FROM salesRank
WHERE worstRank = 1;
...
SELECT 
  v.vendor_name,
  p.product_name,
  SUM(5 * vp.original_price) AS projected_revenue
FROM vendor_products vp
CROSS JOIN customerNew c
...
VALUES (29, 'Apple Pie', 'Large', 3,'unit' ,CURRENT_TIMESTAMP);
...
where product_name = 'Apple Pie' and snapshot_timestamp =(select min(snapshot_timestamp) from product_units where product_name = 'Apple Pie' )
...
WITH latest_quantity AS (
    SELECT vi.product_id, vi.quantity FROM vendor_inventory vi
    INNER JOIN (
        SELECT product_id, MAX(market_date) AS last_date FROM vendor_inventory
        GROUP BY product_id
    ) latest
    ON vi.product_id = latest.product_id AND vi.market_date = latest.last_date)
UPDATE product_units
SET current_quantity = COALESCE((
    SELECT li.quantity FROM latest_inventory li
    WHERE li.product_id = product_units.product_id), 0);
